...
module PetShopServer
  ...
  def self.seed_tables
    ...
    cats_by_shop.each do |shop|
      cat_shop += 1
      shop.each do |cat|
        cats_query << "(\'" + db.escape_string(cat["name"]) + "\', \'" + (cat["adopted"].to_s != "t" ? "f" : "#{cat["adopted"].to_s}") + "\', \'" + db.escape_string(cat["imageUrl"]) + "\', \'#{cat_shop.to_s}\')"
      end
    end
    ...
    dogs_by_shop.each do |shop|
      ...
      shop.each do |dog|
        ...
        dogs_query << "(\'" + db.escape_string(dog["name"]) + "\', \'" + (dog["adopted"].to_s != "t" ? "f" : "#{dog["adopted"].to_s}") + "\', \'" + db.escape_string(dog["imageUrl"]) + "\', \'#{dog_shop.to_s}\')"
      end
    end
